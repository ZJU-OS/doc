...
在此前的实验中，我们的操作系统是完全运行在内存中的，所有数据仅在运行时存在，系统重启后数据全部丢失。我们加载用户态的 ELF 文件时，也是通过将它们预先写入磁盘映像文件，然后在内核中将它们读入内存来实现的。

在本次实验中，我们将实现一个简单的 FAT32 文件系统驱动，使得内核能够读写文件系统镜像中的文件内容，实现持久化存储。
...
在 Unix 哲学中有着 **“Everything is a File“** 的设计思想，一切实现了 `struct file_operations` 接口的操作系统对象都可以被视为“文件“，可以通过统一抽象的文件接口对它们进行操作。最简单也最符合直觉的例子就是数据文件，例如 `vmlinux` 镜像、`.c` 代码文件等，我们可以用 `read`, `write` 等方式对它们进行读写；而事实上，任何有驱动程序的设备、进程的地址空间、管道等都可以被抽象成“文件“来进行操作。

在本次实验中，我们只会涉及 FAT32 文件系统中的文件读写、std 标准输入输出功能，因此我们在 `kernel/include/fs.h` 中定义如下的文件抽象：
...
可以看到，文件抽象包含了文件描述符、权限、当前偏移等通用属性，以及指向具体文件系统实现的文件结构体 (如果有)。最重要的是，文件抽象包含了一系列**文件操作的函数指针**，不同类型的文件可以实现不同的文件操作，而通过文件抽象的统一接口来调用。
...
这样一来，每个进程都拥有自己的文件描述符表，可以打开和管理多个文件。当前进程总是可以通过 `current->files` 访问自己的文件并进行文件操作。
