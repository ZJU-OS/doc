...
!!! example "动手做/思考题"
    ...
    「动手做」是一些需要修改代码、动手操作，但不属于实验任务的内容，要求完成并记录到实验报告中。
    
    「思考题」可能涉及对实验、项目代码、机制的分析解释，需要你自行思考并在实验报告中回答。
...
[浙江大学 2025—2026 学年校历](https://bksy.zju.edu.cn/2025/0704/c28435a3067627/page.htm)
...
Lab3-5 允许**两人**组队完成，要求如下：

- **分工**：
    - 主要目的是**防止只有一个人干活**的情况，要求组队的同学参与到每个实验中。
    - 分工方式自由组队成员自行决定。可以一起完成整个实验，也可以分别做不同的部分相互对接。
- **代码**：
    - 实验完成时，两位同学的代码仓库中，对应的分支应当一致。
    - 要求两位同学都有提交记录。根据 Git Commit 中的邮箱判定，**请将提交邮箱设置为自己的学号邮箱**。
    - 若无提交记录则视为未完成该实验。
- **验收**：
    - 要求两位同学同时到场验收。
    - 需要说明分工情况，如果是一起完成则随意挑一部分讲，如果是各自完成则讲清楚各自负责的部分。
    - 若无法讲清楚代码在干什么则视为未完成该实验。
- **报告**：要求两位同学各自撰写。需要说明分工情况，讲清楚自己做了哪些事情。
...
- **推送代码并附带链接：**如果是代码相关的问题，建议**将代码推送到远程仓库**，并附带链接，这样他人可以**方便地查看你的完整代码**。
- **描述清楚问题：**请清楚地描述你遇到的问题，包括你**期望的行为、实际发生的行为以及你已经尝试过的解决方法**。

助教不接受 DDL 当天的求助请求，请提前规划时间，避免在最后一刻遇到问题。

## 反馈与贡献

如果你发现课程文档、代码有错误、缺陷、值得进一步完善的地方等，请向助教反馈。如果你有改进的想法，欢迎你在 GitHub/ZJU Git 上向课程仓库提交 Merge Request，注意不要提交自己的私有代码。
