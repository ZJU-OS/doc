...
2. **什么是上下文？如何保存和恢复？**（理论课第三章内容 - 进程）
    ...
    这一步，我们需要为每个 Task 设计一个数据结构（即理论课介绍的 PCB，Process Control Block），用于保存任务的寄存器上下文。

3. **调度器应该如何工作？**（理论课第五章内容 - 调度算法）

    有了内存管理和上下文保存机制，接下来就需要实现调度器。调度器的任务是：

    1. 根据调度算法选择下一个要运行的任务
    2. 调用上下文切换代码，将 CPU 控制权交给它

    这一步我们继续完善 Task 数据结构，添加调度相关的属性：

    - 任务状态（运行/就绪/结束）
    - 优先级等调度属性

    并将它们组织成一个**双向链表**，供调度器统一管理。

    - **如何设计调度算法？最简单的轮转调度、优先级调度如何实现？**
    - **我们应该在何处保存和恢复上下文？Trap 入口？调度器？**
