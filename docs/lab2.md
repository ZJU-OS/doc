...
??? tip "多核处理器 (SMP) 带来的新挑战"
    ...
    因此，在多核环境中，关闭中断**不是一个有效**的同步机制。我们必须引入更强大的**锁**，例如**自旋锁 (Spin Locks)**，来确保在任何时刻只有一个 CPU 核心能进入特定的临界区。
...
- **调度顺序**：
    ...
    - 当进程的时间片耗尽，从**当前进程** (current) 开始向后寻找下一个就绪进程进行调度。（双向循环链表其实是一个环，按图示方式进行循环遍历即可维护 FCFS 的调度顺序）
    <figure markdown="span">
        ![scheduler_list.drawio](lab2.assets/scheduler_list.drawio)
        <figcaption>
        调度器中的进程队列示意图
        </figcaption>
    </figure>
- **时间片管理**：
    ...
    - 在每次 `schedule()` 调用时，应当**计算调度间隔**，并对当前进程的调度时间并进行更新。

    !!! tip "计算调度间隔的方法"

        - 我们可以维护一个**静态本地变量**记录上次调度时间，并在每次调度时根据当前时间计算调度间隔。
        - 在计算调度间隔时，我们并没有严格地区分调度器执行的时间开销和进程执行的时间，因此可以**原子地**把“当前时间”同时作为上一进程的结束时间和下一进程的开始时间，如下所示：
        ```c title="kernel/arch/riscv/kernel/sched.c"
        static uint64_t last_sched_time = 0;

        uint64_t elapsed_time(void)
        {
            if (last_sched_time == 0) {
                last_sched_time = clock();
                return 0;
            }
            uint64_t now = clock();
            uint64_t elapsed = now - last_sched_time;
            last_sched_time = now;
            return elapsed;
        }
        ```
...
- **进程资源管理**：
    - 在调度器中，应当确保已退出进程（`TASK_DEAD`）的资源被正确释放。
- **调度间隔**：
    - 调度器在任意 `trap` 处理结束时都会被调用（如系统调用、**时钟中断**），其他进程也可能会因为阻塞等原因主动调用 `schedule()`。
    - 但无论如何，时钟中断都是触发调度的重要手段，在本实验中我们依赖于时钟中断进行进程抢占，为了更精确地更新进程时间片，你需要修改时钟中断频率，从原来的 1s 改为 `kernel/arch/riscv/include/sched.h` 中定义的 `TIMER_INTERVAL` (0.01s)。
