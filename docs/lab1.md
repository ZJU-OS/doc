...
!!! info "FAQ：为什么在后续的实验中，栈不需要 16B 对齐？"

    有同学注意到 RISC-V ABI 规范约定栈 sp 需要 16B 对齐，而后文做栈指针操作时却没有考虑对齐。

    从源头上看，**16B 对齐的需求来自 RISC-V Q 指令集扩展，它支持 128bit 浮点数**。为了在 ABI 层面保证对各指令集扩展的广泛支持，ABI 要求 RV32/64 的栈均做 16B 对齐。再往外推一步，这个 16B 最早来自 x86 架构的 SSE 指令集扩展，此后 Arm 架构的 NEON 指令集也是如此，因此该约定被不同架构的 ABI 广泛采用。

    我们考虑该约定造成的开销。栈上存的每个元素一般至少是寄存器/指针大小，在 RV64 是 8B，所以多了 1 个元素的开销；而在 RV32 中是 4B，这就多了 3 个元素的开销。在嵌入式领域，每一个字节的空间都很重要，因此该约束也曾有过争论（见 [Towards a "golden model" of the RISC-V calling convention(s)](https://groups.google.com/a/groups.riscv.org/g/sw-dev/c/SFcqfIrRhQc)）。如果你要写一个正常的 Linux 程序，那么应该遵守 16B 对齐约定，因为软件生态中的其他依赖都假设彼此遵守约定，否则会引发极难调试的 Bug。

    但对于本课程实验来说：

    - 我们的所有内核态、用户态代码都是自己写的，自成一派了不需要考虑和其他软件的兼容性
    - 没用到 Q 指令集
