...
为了实现不同进程的地址空间隔离，每个进程都需要有自己的页表。在进程生命周期的各个阶段，我们都需要维护页表。先考虑进程的创建阶段：

- PID 0：最初的进程，应当将其页表设置为内核页表 `swapper_pg_dir`。
- 其他进程：都是由 `copy_process()` 创建的，应当将父进程的页表进行**深拷贝**，得到子进程的页表。

页表是三级树形结构，最底层的 PTE 指向相应的物理页。对于这样的间接引用结构，存在两种拷贝方式：

- **浅拷贝（Shallow Copy）**：仅拷贝页表结构本身，PTE 仍然指向相同的物理页。

    浅拷贝一般需要配合**引用计数**使用，可以联想 C++ 语言中的共享指针（`shared_ptr`）。

- **深拷贝（Deep Copy）**：不仅拷贝页表结构，还为每个 PTE 指向的物理页分配新的物理页，并将原物理页的内容复制到新页中。
...
- `sp`：应当填充用户栈的初始值，见第一节对用户空间内存布局的介绍。
